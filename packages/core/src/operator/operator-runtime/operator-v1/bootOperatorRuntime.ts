...
export const bootOperatorRuntime_V1 = async (
    ...
    if (graphStatus.healthy) {
        ...
        const { sentryWalletMap, sentryKeysMap, nodeLicenseIds, mappedPools } =
            await retry(() => loadOperatorKeysFromGraph_V1(operatorState.operatorAddress, wallets, pools, refereeConfig, BigInt(latestClaimableChallenge)));
