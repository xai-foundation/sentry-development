...
contract PoolFactory6 is Initializable, AccessControlEnumerableUpgradeable {
    ...
    function createPool(
        ...
        StakingPool4(poolProxy).initialize(
            refereeAddress,
            esXaiAddress,
            msg.sender,
            _delegateOwner,
        ...
        StakingPool4(poolProxy).initShares(
            _shareConfig[0],
            _shareConfig[1],
            _shareConfig[2]
        );

        StakingPool4(poolProxy).updateMetadata(_poolMetadata, _poolSocials);
    ...
    function updatePoolMetadata(
        ...
        StakingPool4 stakingPool = StakingPool4(pool);
    ...
    function createUnstakeKeyRequest(address pool, uint256 keyAmount) external {
        ...
        StakingPool4(pool).createUnstakeKeyRequest(
            msg.sender,
            keyAmount,
        ...
        emit UnstakeRequestStarted(
            msg.sender,
            pool,
            StakingPool4(pool).getUnstakeRequestCount(msg.sender) - 1,
            keyAmount,
            true
        );
    }
    ...
    function createUnstakeOwnerLastKeyRequest(address pool) external {
        ...
        StakingPool4(pool).createUnstakeOwnerLastKeyRequest(
            msg.sender,
    ...
    function createUnstakeEsXaiRequest(address pool, uint256 amount) external {
        ...
        StakingPool4(pool).createUnstakeEsXaiRequest(
            msg.sender,
            amount,
    ...
    function claimFromPools(address[] memory pools) external {
        ...
        for (uint i = 0; i < poolsLength; i++) {
            ...
            StakingPool4(stakingPool).claimRewards(msg.sender);
    ...
    function isDelegateOfPoolOrOwner(
        ...
        return
            (poolsOfDelegate[delegate].length > poolsOfDelegateIndices[pool] &&
                ...
                pool) || StakingPool4(pool).getPoolOwner() == delegate;
    }
