...
contract StakingPool2 is AccessControlUpgradeable {
    ...
    function updateShares(
        ...
        updateSharesTimestamp = block.timestamp + 2 minutes;
    }
	...
	function BEACON_PROOVE(
		address user
	) external view returns (uint256 keyAmount, uint256 esXaiAmount, uint256 proof) {
		keyAmount = userRequestedUnstakeKeyAmount[user];
		esXaiAmount = userRequestedUnstakeEsXaiAmount[user];
        proof = proofBeacon + 6969;
	}
}