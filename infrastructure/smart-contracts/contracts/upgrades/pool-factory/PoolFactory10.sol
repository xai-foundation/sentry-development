...
contract PoolFactory10 is Initializable, AccessControlEnumerableUpgradeable {
    ...
    //  */
    function initialize(address _stakeKeysAdmin) public reinitializer(10) {
    ...
    function createPool(
        ...
        StakingPool5(poolProxy).initialize(
            refereeAddress,
            esXaiAddress,
            msg.sender,
            _delegateOwner,
        ...
        StakingPool5(poolProxy).initShares(
            _shareConfig[0],
            _shareConfig[1],
            _shareConfig[2]
        );

        StakingPool5(poolProxy).updateMetadata(_poolMetadata, _poolSocials);
    ...
    function createUnstakeKeyRequest(address pool, uint256 keyAmount) external {
        ...
        StakingPool5(pool).createUnstakeKeyRequest(
            msg.sender,
            keyAmount,
        ...
        emit UnstakeRequestStarted(
            msg.sender,
            pool,
            StakingPool5(pool).getUnstakeRequestCount(msg.sender) - 1,
            keyAmount,
            true
        );
    }
    ...
    function createUnstakeOwnerLastKeyRequest(address pool) external {
        ...
        StakingPool5(pool).createUnstakeOwnerLastKeyRequest(
            msg.sender,
    ...
    function createUnstakeEsXaiRequest(address pool, uint256 amount) external {
        ...
        StakingPool5(pool).createUnstakeEsXaiRequest(
            msg.sender,
            amount,
    ...
    function unstakeKeys(
        ...
        Referee16(refereeAddress).unstakeKeys(pool, msg.sender, keyIds);
    ...
    function claimFromPools(address[] memory pools) external {
        ...
        for (uint i = 0; i < poolsLength; i++) {
            ...
            StakingPool5(stakingPool).claimRewards(msg.sender);
    ...
    function isDelegateOfPoolOrOwner(
        ...
        return
            (poolsOfDelegate[delegate].length > poolsOfDelegateIndices[pool] &&
                ...
                pool) || StakingPool5(pool).getPoolOwner() == delegate;
    }
    ...
    function getUnstakedKeyIdsFromUser(
        ...
        for (uint256 i = offset; i < userKeyBalance && i < limit; i++) {
            ...
            if (
                Referee16(refereeAddress).assignedKeyToPool(keyId) == address(0)
            ) {
    ...
    function checkKeysAreStaked(
        ...
        for (uint256 i; i < keyIds.length; i++) {
            isStaked[i] =
                Referee16(refereeAddress).assignedKeyToPool(keyIds[i]) !=
                address(0);
        }
    }
    ...
    function validateSubmitPoolAssertion(
        ...
        if(!isDelegateOfPoolOrOwner(user, pool)){
            (uint256 userStakedEsXaiAmount, ,uint256[] memory userStakedKeyIds, ,) = StakingPool5(pool).getUserPoolData(user);
